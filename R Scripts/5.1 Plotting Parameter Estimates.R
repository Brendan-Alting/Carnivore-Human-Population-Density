...
Coefficientsnoref <- Coefficients[-c(1,6,11),] #remove rows that are the 'reference' levels 

###Assign factors for ggploting in order. 
Coefficientsnoref$Covariate <- factor(Coefficientsnoref$Covariate, levels = c("Session 4", "Session 3",  "Session 2", "Walking Trail", "Road", "Shoulder Season", "ARS"))
Coefficientsnoref$Cov <- factor(Coefficientsnoref$Cov, levels = c("Total", "Session", "Anthropogenic Resource Supplement", "Season", "TrailType"))
...
CoefficientPlotNoRef <- ggplot(Coefficientsnoref, aes(x = Estimate, y = Covariate, color = Cov))+
  ...
  scale_colour_manual(name = "Parameter",
                      values = c("Session" = "red", "Anthropogenic Resource Supplement" = "blue", "Season" = "orange", "TrailType" = "green"),
                      labels = c("Session" = "Session",  "Anthropogenic Resource Supplement"="Anthropogenic\nResource Supplement", "Season" = "Season", "TrailType" = "TrailType")) +
...
png("Figures/Manuscript Figures/SplitCoefficientsColourcov.png", width = 12, height = 8, res= 600, units = "in")
